...
global Version := "v20200915"	; The version number of this script
...
global Workdir = A_WorkingDir
global BootImgPath := Workdir . "\boot.bmp"	; set your Old Boot image path here
global ABootImgPath := Workdir . "\Aboot.bmp"	; set your Ancient Boot image path here
global TraserImgPath := Workdir . "\traser.bmp"	; set your Traser image path here
global ClickImgPath := Workdir . "\click.bmp" ; set your Traser image path here
...
global Flag_winhide := False ;if 0 were show, if 1 were hide
...
L_hide:
    ...
    {
        Flag_winhide := True
        WinGet, Hide_win_id , ID , A
        WinHide, ahk_id %Hide_win_id%
        UpdateTooltip()
    }
    else
    {
        Flag_winhide := False
        WinShow ahk_id %Hide_win_id%
        UpdateTooltip()
    }
Return
...
UpdateTooltip() {
    ...
    if (!Flag_winhide) {
        Info_hide := "`n[" . HK_hidewin . "] Hide-window is OFF."
    } else if (Flag_winhide) {
        Info_hide := "`n[" . HK_hidewin . "] Hide-window is ON."
    }
